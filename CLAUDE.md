...
- **阶段一 · 显示（做到好看为止）**
  ...
  - 1.2 特效升级（✅ 四个 WebGL2 特效 + 后期处理 + 鼠标交互）：
    ...
    - ⏳ 待深化：flowfield 速度上色（现按 `v_age` 渐变，改按粒子速度染色）、四化点星（把四化标到星盘对应星位，区别于已做的 `enhanceSihua` 彩色药丸）、`navigator.gpu`→WebGPU 增量（明确缓，WebGL2 基线已达标）。（真流体墨、bento 布局已完成——`ink.js` 半拉格朗日平流真流体、`enhanceBento` 通栏页脚。）
...
- **下一步**：阶段三 Windows 壁纸层（WorkerW）。⏳ 遗留：Win WASAPI 音频验证、阶段一 ⏳ 待深化项（flowfield 速度上色/四化点星/WebGPU 增量）、开机自启（阶段四，需签名 `.app`）、每屏配置在设置窗打开时热插拔的屏列表刷新（当前仅开窗/切预设时刷新）。
